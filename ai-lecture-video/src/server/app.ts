...
export async function createServer(
  ...
  server.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const resumeStates =
        job.failed_module && job.run_directory
          ? moduleStatesForRetry(job.modules, job.failed_module)
          : undefined;
      const updated =
        job.failed_module && job.run_directory && resumeStates
