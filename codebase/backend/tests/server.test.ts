...
test("duration failure retries from script generation and bypasses stale cache", async (t) => {
  const directory = await temporaryDirectory(t);
  ...
  modules.module3_script_generator = { status: "COMPLETED" };
  modules.module4_storyboard_generator = { status: "COMPLETED" };
  modules.module5a_visual_generator = { status: "COMPLETED" };
  modules.module5b_voice_generator = { status: "COMPLETED" };
  modules.module6_video_composer = {
    status: "FAILED",
    error:
  ...
  const job: JobRecord = {
    ...
    warnings: [],
    modules,
    failed_module: "module6_video_composer",
    resume_from: "module6_video_composer",
    error:
      "DURATION_OUT_OF_RANGE: audio thực tế 129.53s, yêu cầu 180-300s.",
  };
  await writeFile(
  ...
  assert.equal(response.json().retry_from, "module3_script_generator");
  ...
  assert.equal(stored.bypass_generation_cache, true);
...
test("saved document serves PDF and summary, then starts video from module 2", async (t) => {
  ...
  const runDirectory = path.join(directory, "runs", "document-run");
  await mkdir(jobsDirectory, { recursive: true });
  await mkdir(uploadsDirectory, { recursive: true });
  await mkdir(runDirectory, { recursive: true });
  const inputFile = path.join(uploadsDirectory, "document-job.pdf");
  await writeFile(inputFile, "%PDF-1.4\n%%EOF");
  await writeFile(
    path.join(runDirectory, "07_summary.json"),
    JSON.stringify({
      schema_version: "1.0",
      title: "Tóm tắt tài liệu",
      overview: "Tổng quan có căn cứ.",
      key_points: [
        {
          heading: "Ý chính",
  ...
  const summary = await server.inject({
    method: "POST",
    url: `/api/documents/${job.id}/summary`,
  });
  assert.equal(summary.statusCode, 200);
  assert.equal(summary.json().key_points[0].source_pages[0], 1);

  const video = await server.inject({
    method: "POST",
    url: `/api/documents/${job.id}/video`,
    payload: {
      title: "Video mới",
      aspect_ratio: "16:9",
      duration_option: "3-5",
  ...
  assert.equal(video.statusCode, 202);
  assert.equal(video.json().kind, "VIDEO");
  assert.equal(video.json().retry_from, "module2_lecture_planner");
  assert.equal(
    video.json().modules.module1_document_intelligence.status,
    "COMPLETED",
  );
  assert.equal(
